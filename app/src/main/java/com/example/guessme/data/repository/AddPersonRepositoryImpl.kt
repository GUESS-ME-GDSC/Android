...
class AddPersonRepositoryImpl @Inject constructor(
    ...
    override suspend fun addPerson(token: String, person: Person, image: File?): Response<BaseResponseBody> {
        var imageRequestBody: RequestBody?
        var imageMultipartBody: MultipartBody.Part? = null
        image?.let {
            imageRequestBody = image.asRequestBody("image/jpeg".toMediaTypeOrNull())
        ...
        person.voice?.let {
            ...
            voiceRequestBody = voiceFile!!.asRequestBody("audio/mp4".toMediaTypeOrNull())
            voiceMultipartBody = MultipartBody.Part.createFormData(name= "image", filename = voiceFile!!.name, body = voiceRequestBody!!)
        }

        val nameRequestBody = person.name.toRequestBody("text/plain".toMediaTypeOrNull())
        val relationRequestBody = person.relation.toRequestBody("text/plain".toMediaTypeOrNull())
        val birthRequestBody = person.birth.toString().toRequestBody("text/plain".toMediaTypeOrNull())
        val residenceRequestBody = person.residence.toRequestBody("text/plain".toMediaTypeOrNull())
